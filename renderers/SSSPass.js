(function() {
    class SSSPass {
        constructor(scene, camera, width, height) {
            ...
            let _renderTargetIrradiance = new THREE.WebGLRenderTarget( this.width * 2, this.height * 2, 
                { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter} );
            _renderTargetIrradiance.texture.format = THREE.RGBAFormat;
            _renderTargetIrradiance.texture.type = THREE.HalfFloatType;

            let _renderTargetSpecular = new THREE.WebGLRenderTarget( this.width * 2, this.height * 2,
                { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter, format: THREE.RGBFormat } );

            let _renderTargetSSSAlbedo = new THREE.WebGLRenderTarget( this.width * 2, this.height * 2, 
                { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter, format: THREE.RGBFormat } );

            let _renderTargetDepth = new THREE.WebGLRenderTarget( this.width * 2, this.height * 2, 
                { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter } );
            _renderTargetDepth.texture.format = THREE.RGBAFormat;
            _renderTargetDepth.texture.type = THREE.FloatType;

            this.renderTargetDepth = _renderTargetDepth;

            let _renderTargetFirstPass = new THREE.WebGLRenderTarget( this.width * 2, this.height * 2, 
                { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter } );
            _renderTargetFirstPass.texture.format = THREE.RGBAFormat;
            _renderTargetFirstPass.texture.type = THREE.HalfFloatType;
            ...
            this.setRTMaterialParam = function(param) {
                Object.assign(_rtMateiralProp, param);
            ...
            this.generateNoiseTexture = function() {
                ...
                if ( THREE.SimplexNoise === undefined ) {
                    console.error( 'THREE.SSSPass: The pass relies on THREE.SimplexNoise.' );
                }
            ...
            const _blurXMaterial = new THREE.ShaderMaterial({
                defines: Object.assign( {}, THREE.SSSShader.defines ),
                uniforms: THREE.UniformsUtils.clone( THREE.SSSShader.uniforms ),
                vertexShader: THREE.SSSShader.vertexShader,
                fragmentShader: THREE.SSSShader.fragmentShader,
                depthWrite: false,
            });

            _blurXMaterial.defines[ 'FIRST_PASS' ] = 1;
            _blurXMaterial.defines[ 'KERNEL_SIZE' ] = _sample_len;
            _blurXMaterial.uniforms[ 'tDiffuse' ].value = _renderTargetIrradiance.texture;
            _blurXMaterial.uniforms[ 'tSpecular' ].value = _renderTargetSpecular.texture;
            _blurXMaterial.uniforms[ 'sssAlbedo' ].value = _renderTargetSSSAlbedo.texture;
            _blurXMaterial.uniforms[ 'tDepth' ].value = _renderTargetDepth.texture;
            ...
            _blurXMaterial.uniforms[ 'ProjectionMatrix' ].value = this.camera.projectionMatrix ;
            ...
            _blurYMaterial.uniforms[ 'tDiffuse' ].value = _renderTargetFirstPass.texture;
            _blurYMaterial.uniforms[ 'tSpecular' ].value = _renderTargetSpecular.texture;
            _blurYMaterial.uniforms[ 'sssAlbedo' ].value = _renderTargetSSSAlbedo.texture;
            _blurYMaterial.uniforms[ 'tDepth' ].value = _renderTargetDepth.texture;
            ...
            _blurYMaterial.uniforms[ 'ProjectionMatrix' ].value = this.camera.projectionMatrix ;
            ...
            this.render = function(renderer, writeBuffer) {
                ...
                renderer.setRenderTarget( _renderTargetIrradiance );
                // renderer.setRenderTarget( writeBuffer );
                renderer.clear();
                renderer.render( this.rtScene, this.camera );


                _rtMeshes.forEach((obj) => {
                    obj.material = _rtSpecularMaterial;
                });
                renderer.setRenderTarget( _renderTargetSpecular );
                //renderer.setRenderTarget( writeBuffer );
                renderer.clear();
                renderer.render( this.rtScene, this.camera );
                ...
                renderer.setRenderTarget( _renderTargetSSSAlbedo );
                // renderer.setRenderTarget( writeBuffer );
                renderer.clear();
                renderer.render( this.rtScene, this.camera );
                ...
                renderer.setRenderTarget( _renderTargetDepth );
                //renderer.setRenderTarget( writeBuffer );
                renderer.clear();
                renderer.render( this.rtScene, this.camera );

                _quad.material = _blurXMaterial;
                renderer.setRenderTarget( _renderTargetFirstPass );
                // renderer.setRenderTarget( writeBuffer );
                renderer.clear();
                renderer.render( _blurScene, this.camera );
